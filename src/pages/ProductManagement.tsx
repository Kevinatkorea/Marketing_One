...
export default function ProductManagement() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editTarget) {
        await updateProduct(id, editTarget.id, {
          name: form.name,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-zinc-100">상품/서비스 관리</h1>
          ...
          products.map((p) => (
            <div key={p.id} className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 sm:p-5 relative group">
              <div className="absolute top-3 right-3 flex gap-1">
                <button
                  onClick={() => openEdit(p)}
                  className="w-7 h-7 rounded-md flex items-center justify-center text-zinc-500 hover:text-blue-400 hover:bg-blue-500/10 bg-zinc-800/80 border border-zinc-700/50 transition-all z-10"
                  title="수정"
                >
                ...
                <button
                  ...
                  className="w-7 h-7 rounded-md flex items-center justify-center text-zinc-500 hover:text-red-400 hover:bg-red-500/10 bg-zinc-800/80 border border-zinc-700/50 transition-all z-10"
                  title="삭제"
                >
              ...
              <h3 className="text-sm font-semibold text-zinc-100 pr-16">{p.name}</h3>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowModal(false)}>
          <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-4 sm:p-6 w-full max-w-md mx-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-bold text-zinc-100 mb-4">{editTarget ? '상품 수정' : '상품 등록'}</h2>
            ...
            <div className="flex justify-end gap-2 mt-6">
              ...
              <button onClick={handleSubmit} disabled={submitting || !form.name.trim()}
                ...
                {submitting ? '저장 중...' : editTarget ? '수정 완료' : '상품 등록'}
              </button>
            </div>
          </div>
        </div>
